...
public class SysUserController extends BaseController
{
     ...
     */
    @GetMapping("/treelist")
    public AjaxResult treelist(SysUser user2)
    {
        List<SysUser> list = userService.selectUserList(user2);
        ...
        if(!rolenames.contains(Const.ROLE_ADMIN)){
            ...
            if(rolenames.size()==1 && rolenames.contains(Const.ROLE_COMMON)){
                ...
                Map map = null;
                final val collect = list.stream().filter(o->o.getDeptId().equals(user2.getDeptId())).collect(Collectors.groupingByConcurrent(a->a.getDept().getDeptName()));
            ...
            //最高级别
                ...
                List<Map> result = new ArrayList();
                Map map = null;
                final val collect = list.stream().collect(Collectors.groupingByConcurrent(a->a.getDept().getDeptName()));
                for(String key:collect.keySet()){
                    map = new HashMap();
                    map.put("deptname",key);
                    map.put("children",collect.get(key));
